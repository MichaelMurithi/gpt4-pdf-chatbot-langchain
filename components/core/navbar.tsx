    ...
    return (
        ...
        <div className='flex flex-row justify-between'>
          <div className='flex flex-row'>
           <Avatar image={data!.user.image ?? ''} shape="circle" aria-label='User' size='normal' />
           <p className="ml-2 mt-1">{data?.user.name}</p>
          </div>
          <div className='flex flex-row'>
            <i className="mt-1 m-0 pi pi-cog" aria-label="Settings" />
            <i className="mt-1 ml-2 m-0 pi pi-angle-left" aria-label="Settings" onClick={() => toggleVisibility(false)}/>
          </div>
        </div>
        <div className='mt-8'>
          <div className='flex flex-row justify-between'>
